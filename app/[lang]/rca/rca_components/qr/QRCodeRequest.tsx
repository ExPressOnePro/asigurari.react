...
const QRCodeRequest: React.FC<QRCodeRequestProps> = React.memo(({ onStepChange, step }) => {
            ...
            if (!isMounted.current && selectedInsurer?.PrimeSumMDL) {
                ...
                try {
                    ...
                    if (response.data?.uuid) {
                        ...
                        // Здесь вы меняете шаг на 5
                        onStepChange(5);  // Переход к следующему шагу
                    } else {
    ...
    }, [selectedInsurer?.PrimeSumMDL]); // убрали `step`, чтобы эффект не запускался при каждом изменении шага
